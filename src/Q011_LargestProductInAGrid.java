/*
 * In the [20 X 20] grid below, four numbers along a diagonal line have been marked with a star.
...
public class Q011_LargestProductInAGrid {
	...
	private static void prodDaigRight(int i, int j) {
		int prod = 0;
		if(i <= 16 && j <= 16)
	...
	private static void prodDaigLeft(int i, int j) {
		int prod = 0;
		if(i <= 16 && j >= 3)
			prod = arr[i][j] * arr[i+1][j-1] * arr[i+2][j-2] * arr[i+3][j-3];
		if(maxProd < prod)
	...
	private static void prodUp(int i, int j) {
		int prod = 0;
		if(i >= 3)
	...
	private static void prodLeft(int i, int j) {
		int prod = 0;
		if(j >= 3)
	...
	private static void prodDown(int i, int j) {
		int prod = 0;
		if(i <= 16)
			prod = arr[i][j] * arr[i+1][j] * arr[i+2][j] * arr[i+3][j];
		if(maxProd < prod)
	...
	private static void prodRight(int i, int j) {
		int prod = 0;
		if(j <= 16)
			prod = arr[i][j] * arr[i][j+1] * arr[i][j+2] * arr[i][j+3];
		if(maxProd < prod)
